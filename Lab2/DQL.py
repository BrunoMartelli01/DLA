...
a convolutional feature extractor and a fully-connected head.
    1. Convolutional Base: This part of the network processes the input image to identify important
        ...
        `nn.Conv2d`: Three sequential convolutional layers with decreasing kernel sizes and strides
                    (`8x8`, `4x4`, `3x3`). The input has 4 channels, representing the four stacked historical frames,
                    which allows the network to infer dynamic information like the direction and speed of objects.
       `F.relu`: ReLU activation is applied after each convolutional layer to introduce non-linearity.
    ...
    * The function returns a tensor of actions, one for each state in the input batch. These actions can then be passed
    ...
    def update_model(self):
        ...
        Goal
            ...
            Q-values and a "target" Q-value. This process, repeated thousands of times, is what allows
        ...
        Method
            ...
            3. Calculate the Target Q-values: This is what the network should think the value is,
                ...
                *   self.target_net(next_state).max(1)[0]: Target_net is used to estimate
...
class StayOnTrack(gym.Wrapper):
    """
    Goal
        To reshape the reward signal so the agent is explicitly incentivized to stay on the road and
        penalized for driving on grass. The default CarRacing-v3 environment applies a flat -0.1 penalty
        per frame regardless of the vehicle's position on the track. This undifferentiated signal does
        not discourage off-track excursions, so the agent may learn to cut corners or drift onto the
        grass without adequate consequence.

    Method
        1. Neutralizing the Default Per-Frame Penalty: In the step method, +0.1 is immediately added
           to the environment's raw reward, effectively canceling the -0.1 per-frame cost and producing
           a neutral baseline from which position-based bonuses and penalties are then applied.
        2. Grass Detection via Pixel Sampling: A small rectangular patch of the observation frame,
           obs[75:80, 42:53], is sampled. This region corresponds to the area directly beneath the
           vehicle in the rendered image. The mean RGB value of this patch is computed across all
           sampled pixels.
        3. Off-Track Classification: The green channel of the mean RGB is used as the discriminating
           feature. A threshold of 135 was determined empirically, values above it reliably indicate
           the bright green of grass rather than the grey of asphalt. If the threshold is exceeded,
           the off_track_penalty is applied; otherwise, the on_track_reward is added as a small continuous
           bonus for remaining on the road.

    Result
        The agent receives a strong negative signal whenever it drives off the track and a small but
        consistent positive reinforcement for staying on the asphalt. This asymmetric reward structure
        guides the agent toward safer, track-hugging behavior and discourages the degenerate strategy
        of ignoring road boundaries.
    """

    def __init__(self, env, off_track_penalty=-3.0, on_track_reward=0.5):
        super().__init__(env)
...
def make_env(render=False, continuous=False, max_episode_steps=1500, stay=True):
    ...
    if stay:
        env = StayOnTrack(env, off_track_penalty=-2.0, on_track_reward=0.05)
    ...
    """
    Goal 
        To train a Deep Q-Network (`DQNAgent`) to master the discrete action space version of the 
        `CarRacing-v3` environment. The entire process is to be logged using TensorBoard for later 
        analysis, and the best version of the agent's neural network should be saved automatically 
        to a file.

    Method

        1.  Environment Initialization:
               A custom make_env function is called to create the environment. 
               The parameters are crucial for training:
                    render=False: Disables the graphical user interface (GUI) during training 
                                  to significantly speed up the process.
                    continuous=False: Configures the environment to have a  discrete set of actions  
                                      instead of continuous joystick-like controls.
                    max_episode_steps=1500: Sets a time limit for each episode.

        2.  Agent Instantiation:
                An instance of the DQNAgent class is created. This object encapsulates everything 
                the agent needs to learn:
                   The main Q-Network that predicts the value of actions.
                   The Target Network, used to stabilize training.
                   The Replay Buffer, which stores past experiences for learning.
                   The epsilon-greedy strategy for balancing exploration and exploitation.

        3.  Setup for Logging and Checkpointing:
               Unique Naming: A unique string based on the current date and time (datetime) is created.
               TensorBoard Logger: A `SummaryWriter` is created, pointing to a new subdirectory within ./runs/. 
                                   The agent's `train` method will use this `writer` object to log key metrics.
               Save Path: A path is defined for saving the best version of the agent's model.

        4.  Training Execution:
                The `agent.train()` method is called. This single line starts the main training loop. 
                The key parameters control the training process:
                    num_episodes=10000: The agent will be trained for a maximum of 10.000 episodes.
                    early_stop_patience=5: If the agent's performance on its evaluation runs doesn't 
                                            improve for 5 consecutive checks, the training will stop 
                                            automatically to save time.
                    
    Result
        1.  A Trained Model (.pth file): A file is saved to the ./dqn/ directory. This file contains the state dictionary  
                                         of the Q-network at its peak performance.

        2.  TensorBoard Log Files: A new directory is created inside ./runs/. This directory contains the log files 
                                   that can be viewed with tensorboard --logdir=./Lab2/runs/ 
...
if __name__ == '__main__':
    ...
    log_dir = f"./runs/dqn_car_racing_{date}"
    writer = SummaryWriter(log_dir)
    best_model_path = f'./dqn/best_dqn_car_racing_{date}.pth'
    agent.train(num_episodes=10000, early_stop_patience=5, max_len=50, save_path=best_model_path,
                writer=writer)
    writer.close()
